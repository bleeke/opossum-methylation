...
dataDir <- "/camp/home/leekeb/working/Bryony/manuscript/analysis/data/adult_bsseq/allele_specific_methyl_extract/bismark_CpG_report"
...
outDir <- "/camp/lab/turnerj/working/Bryony/manuscript/analysis/data/adult_bsseq/allele_specific_methylKit_out/"

# import sample metadata
setwd(sampleDir)
sample_data <- read_delim("adult_sample_data_sex_allelespecific.txt", delim = "\t") 
...
condition_samples <- sample_data %>%
  tidyr::unite(col = sample_condition, tissue, sex, genome, sep = "_", remove = F) %>%
  filter(sample_condition == condition)
# define sample names
samples <- as.list(condition_samples$sample_id)
# define files
genome <- unique(condition_samples$genome)
files <-as.list(paste0(samples,
                       "_merged_fastq.gz_trimmed_bismark_bt2.",
                       genome,
                       ".deduplicated_sorted.CpG_report.txt"))
# list the input files for logs
print("we will work on files:")
print(files)
# define sample treatment
design <- condition_samples$design
#import
setwd(dataDir)
