...
samples <- as.list(tissues)
files <- as.list(paste0(samples, "_destranded_pooled_df.csv"))
...
plot_cols_1 <- c("#9467BD")
plot_cols_2 <- c("#BCBD22")
...
spots_plot <- ggplot()+
    ...
    scale_x_continuous(breaks = c(regions$start[x]-100,
                       ...
                       limits = c(regions$start[x]-100,
                                  regions$end[x]+100),
                       expand = c(0, 0))+
  scale_colour_manual(values = c(plot_cols_1, plot_cols_2))+
  ...
  setwd(plotDir)
  spots_plot
  ggsave(paste0("gametes_", regions$name[x], "_spotplot.pdf"), width = 7, height = 3, useDingbats=FALSE)
}
