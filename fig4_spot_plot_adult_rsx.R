...
samples <- as.list(paste0(c("female_", "male_"), tissue))
...
meth_df <- dplyr::bind_rows(meth_store, .id = 'source') %>%
  separate(source, into = c("sex", "tissue"))

# make a plot

# colours 
plot_cols_1 <- c("#3A5FCD")
plot_cols_2 <- c("#1a2e68")
...
spots_plot <- meth_df %>%
  ...
  setwd(plotDir)
  spots_plot
  ggsave(paste0(tissue, "_", regions$name[x], "_spotplot.pdf"), width = 7, height = 3, useDingbats = FALSE)
